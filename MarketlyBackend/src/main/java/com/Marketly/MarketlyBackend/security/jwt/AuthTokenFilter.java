...
public class AuthTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        logger.debug("authentication started :{}",request.getRequestURI());
               try{
                   String token=jwtParser(request);
                   if(token!=null && jwtUtils.validateToken(token))  {
                       String userName=jwtUtils.getUserNameViaToken(token);
                   ...
                   // 1. Set the exception on the request for the AuthEntryPoint to access
